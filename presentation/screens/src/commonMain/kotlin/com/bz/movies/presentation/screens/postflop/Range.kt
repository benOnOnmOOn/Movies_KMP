...
sealed class Suitedness {
    ...
    data class Specific(val first: Byte, val second: Byte) : Suitedness()
}
...
fun pairIndices(rank: Byte): List<Int> {
    return buildList(6) {
        for (i in 0..3) {
            for (j in i + 1..3) {
                add(cardPairToIndex((4 * rank + i).toByte(), (4 * rank + j).toByte()))
            }
        }
    }
}

fun nonpairIndices(rank1: Byte, rank2: Byte): List<Int> {
    return buildList(16) {
        for (i in 0..3) {
            for (j in 0..3) {
                add(cardPairToIndex((4 * rank1 + i).toByte(), (4 * rank2 + j).toByte()))
            }
        }
    }
}

fun suitedIndices(rank1: Byte, rank2: Byte): List<Int> {
    return buildList {
        for (i in 0..3) {
            add(cardPairToIndex((4 * rank1 + i).toByte(), (4 * rank2 + i).toByte()))
        }
    }
}

fun offsuitIndices(rank1: Byte, rank2: Byte): List<Int> {
    return buildList(12) {
        for (i in 0..3) {
            for (j in 0..3) {
                if (i != j) {
...
fun indicesWithSuitedness(rank1: Byte, rank2: Byte, suitedness: Suitedness): List<Int> {
    return if (rank1 == rank2) {
        when (suitedness) {
            Suitedness.All -> pairIndices(rank1)
            is Suitedness.Specific -> listOf(
                cardPairToIndex(
                    (4 * rank1 + suitedness.first).toByte(),
                    (4 * rank1 + suitedness.second).toByte()
                )
            )
        ...
        when (suitedness) {
            ...
            is Suitedness.Specific ->
                listOf(
                    cardPairToIndex(
                        (4 * rank1 + suitedness.first).toByte(),
                        (4 * rank2 + suitedness.second).toByte()
                    )
                )
...
fun rankToChar(rank: Byte): Char {
    return when (rank) {
        ...
        9.toByte() -> 'J'
        8.toByte() -> 'T'
        in 0.toByte()..7.toByte() -> ('2' + rank.toInt()).toChar()
        else -> error("Invalid input: $rank ")
    }
}

fun suitToChar(suit: Byte): Char {
    return when (suit) {
        0.toByte() -> 'c'
        1.toByte() -> 'd'
        2.toByte() -> 'h'
...
fun indexToCardPair(index: Int): Pair<Card, Card> {
    ...
    return Pair(card1.toByte(), card2.toByte())
}
    ...
    if (suitedness != suitedness2) {
        ...
        if (rank11 > rank21) {
            for (i in rank21..rank11) {
                setWeight(indicesWithSuitedness(i.toByte(), (i - gap).toByte(), suitedness), weight)
            }
        } else {
        ...
        if (rank12 > rank22) {
            for (i in rank22..rank12) {
                setWeight(indicesWithSuitedness(rank11, i.toByte(), suitedness), weight)
            }
        } else {
            error("Range must be in descending order: $range")
        }
...
private fun Range.updateWithPlusRange(range: String, weight: Float) {
    ...
    if (gap <= 1) {
        ...
        for (i in rank2..<rank1) {
            setWeight(indicesWithSuitedness(rank1, i.toByte(), suitedness), weight)
        }
    }
}
 ...
 */
private fun parseSingleton(combo: String): Triple<Byte, Byte, Suitedness> {
 ...
 */
private fun parseSimpleSingleton(combo: String): Triple<Byte, Byte, Suitedness> {
 ...
 * @throws IllegalStateException if the combo string is not in a valid format.
 */
private fun parseCompoundSingleton(combo: String): Triple<Byte, Byte, Suitedness> {
 ...
 */
fun Range.getWeightPair(rank: Byte): Float {
 ...
 */
fun Range.getWeightSuited(rank1: Byte, rank2: Byte): Float {
 ...
 * @throws IllegalArgumentException if `rank1` or `rank2` is not less than 13 or `rank1` is equal to `rank2`.
 */
fun Range.getWeightOffsuit(rank1: Byte, rank2: Byte): Float {
    require(rank1 in 0..12 && rank2 in 0..12) { "rank1 and rank2 must be between 0 and 12" }
    require(rank1 != rank2) { "rank1 and rank2 must be different" }
...
private fun Range.pairsStrings(result: MutableList<String>) {
    var start: Pair<Byte, Float>? = null

    for (i in 12 downTo -1) {
        val rank = i.toByte()
        val prevRank = (i + 1).toByte()
...
private fun Range.nonPairsStrings(result: MutableList<String>) {
    for (rank1 in 12 downTo 1) {
        if (canUnsuit(rank1.toByte())) {
            highCardsStrings(result, rank1.toByte(), Suitedness.All)
        } else {
            highCardsStrings(result, rank1.toByte(), Suitedness.Suited)
            highCardsStrings(result, rank1.toByte(), Suitedness.Offsuit)
        }
    }
}
 ...
 */
private fun Range.canUnsuit(rank1: Byte): Boolean {
    for (rank2 in 0 until rank1) {
        val sameSuited = isSameWeight(suitedIndices(rank1, rank2.toByte()))
        val sameOffsuit = isSameWeight(offsuitIndices(rank1, rank2.toByte()))
        val weightSuited = getWeightSuited(rank1, rank2.toByte())
        val weightOffsuit = getWeightOffsuit(rank1, rank2.toByte())
...
private fun Range.highCardsStrings(
    ...
    var start: Pair<Byte, Float>? = null
    val (getter: (Byte, Byte) -> List<Int>, suitChar) = when (suitedness) {
...
private fun Range.suitSpecifiedStrings(result: MutableList<String>) {
    ...
    for (rank in 12 downTo 0) {
        if (isSameWeight(pairIndices(rank.toByte()))) continue
        for (suit1 in 3 downTo 0) {
            for (suit2 in (suit1 - 1) downTo 0) {
                val weight =
                    getWeightByCards((4 * rank + suit1).toByte(), (4 * rank + suit2).toByte())
                if (weight > 0.0f) {
                    val rankChar = rankToChar(rank.toByte())
                    val suit1Char = suitToChar(suit1.toByte())
                    val suit2Char = suitToChar(suit2.toByte())
                    val tmp = StringBuilder()
                        .append(rankChar)
                        .append(suit1Char)
                        .append(rankChar)
